...
local function loadConfig()
    ...
    if data.ignoreWalls ~= nil then ignoreWalls[0] = data.ignoreWalls end
    maxDistance[0] = data.maxDistance or maxDistance[0]

    if data.showIcon ~= nil then showIcon[0] = data.showIcon end
    if data.showNick ~= nil then showNick[0] = data.showNick end
    if data.showBar ~= nil then showBar[0] = data.showBar end
    if data.useServerNickColor ~= nil then useServerNickColor[0] = data.useServerNickColor end
...
local function drawBars(sx, sy, id)
    ...
    local hpColor = ffi.new("CRGBA",
        hpCol[0]*255, hpCol[1]*255, hpCol[2]*255, hpCol[3]*255
    )
    local hpBack = ffi.new("CRGBA",
        hpBg[0]*255, hpBg[1]*255, hpBg[2]*255, hpBg[3]*255
    )
    local arColor = ffi.new("CRGBA",
        arCol[0]*255, arCol[1]*255, arCol[2]*255, arCol[3]*255
    )
    local arBack = ffi.new("CRGBA",
        arBg[0]*255, arBg[1]*255, arBg[2]*255, arBg[3]*255
    )

    gta._ZN9CSprite2d12DrawBarChartEffthfahh5CRGBAS0_(
        hpX, hpY, hpWidthScale[0], hpHeightScale[0], hp, 1, 0, 1, hpColor, hpBack
    )

    if ar > 0 then
        gta._ZN9CSprite2d12DrawBarChartEffthfahh5CRGBAS0_(
            arX, arY, arWidthScale[0], arHeightScale[0], ar, 1, 0, 1, arColor, arBack
        )
    end
end
            ...
            and not sampIsPlayerNpc(id) then
                ...
                if bx then
                    ...
                    if dist <= maxDistance[0] then
                        if isVisible(px, py, pz + 0.2, bx, by, bz + 0.2) then
                            ...
                            if ok2 then
                                if showIcon[0] then
                                    ...
                                    if wep == 0 then
                                        local c = ffi.new("CRGBA", color[0]*255, color[1]*255, color[2]*255, color[3]*255)
                                        ...
                                        gta._ZN7CSprite18RenderOneXLUSpriteEfffffhhhsfhhhff(
                                            wx, wy, 10.0, w * 0.5, h * 0.5,
                                            color[0]*255, color[1]*255, color[2]*255, 255,
                                            1.0, color[3]*255, 0, 0, 0.0, 0.0
                                        )
                                    end
                                end
                                ...
                                if showNick[0] then
                                    ...
                                    drawText(sx, sy + nickY, name, nickColor)
                                end
...
-- ============================================================
-- MONET DPI SCALE (manual, mimgui gak punya GetDPI bawaan)
-- Dihitung dari rasio DisplaySize aktual vs resolusi referensi.
-- ============================================================
local REF_WIDTH  = 1280.0
local REF_HEIGHT = 720.0
local MDS = 1.4

local function computeDPI()
    local ok, io = pcall(imgui.GetIO)
    if not ok or not io then return 1.0 end

    local w = io.DisplaySize.x
    local h = io.DisplaySize.y

    if not w or not h or w <= 0 or h <= 0 then return 1.0 end

    local scale = math.min(w / REF_WIDTH, h / REF_HEIGHT)

    -- clamp biar gak ekstrem kekecilan/kegedean di device aneh
...
imgui.OnInitialize(function()
    ...
    if doesFileExist(FONT_PATH) then
        local ok1, f1 = pcall(io.Fonts.AddFontFromFileTTF, io.Fonts, FONT_PATH, 18 * MDS)
        if ok1 then fontTitle = f1 end
        local ok2, f2 = pcall(io.Fonts.AddFontFromFileTTF, io.Fonts, FONT_PATH, 11 * MDS)
        if ok2 then fontTitleSmall = f2 end
    end

    local iconRanges = imgui.new.ImWchar[3](faicons.min_range, faicons.max_range, 0)
    local okIcon, iconFont = pcall(io.Fonts.AddFontFromMemoryCompressedBase85TTF,
        io.Fonts,
    ...
    if okIcon then fontAwesome = iconFont end
...
function imgui.BeginCustomTitle(title, titleSizeY, var, flags)
    ...
    if opened then
        ...
        local iconTextSize = imgui.CalcTextSize(iconText)

        local iconPos = imgui.ImVec2(
            saveCenter.x - iconTextSize.x/2,
            saveCenter.y - iconTextSize.y/2 + 5 * MDS
        )
        ...
        imgui.SetCursorPosY(titleSizeY + 5 * MDS)
    end
...
function()
    ...
    if imgui.BeginCustomTitle(
        ...
        28 * MDS,
        window,
        imgui.WindowFlags.NoCollapse + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoResize
    ) then

        local function addAdjustInt(label, var, step, minv, maxv)
            imgui.PushItemWidth(280 * MDS)
            imgui.SliderInt("##"..label, var, minv, maxv)
            imgui.PopItemWidth()

            imgui.SameLine()
            if imgui.Button("-##"..label, imgui.ImVec2(29 * MDS, 29 * MDS)) then
...
function ev.onInitGame(playerId, hostName, settings, vehicleModels, vehicleFriendlyFire)
    ...
    if type(vehicleModels) == "table" then
        for i = 1, 212 do
            fixedModels[i] = vehicleModels[i] or vehicleModels[i - 1] or 0
        end
    else
        for i = 1, 212 do
            fixedModels[i] = 0
    ...
    return { playerId, hostName, settings, fixedModels, vehicleFriendlyFire }
end

local function checkScriptName()
